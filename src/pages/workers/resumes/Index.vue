...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getResumes(Object.assign({q_worker_id_eq: this.$root.item.id}, this.mixTableParams)))
    },
    fetchItemFootprints (item) {
      this.api.getResumeFootprints(item.id).then(res => {
      ...
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteResume(row.id).then(res => {
