  ...
  <div>
    <ex-table :tableData="mixTableData" @refetch="fetchData">
      ...
      <template slot="table-operations" slot-scope="{row, $index, intro}">
        ...
        <el-button size="mini" @click="showResumeDialog(row)">简历</el-button>
        <el-button size="mini" @click="showCommentDialog(row)">评价</el-button>
...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getWorkers(this.mixTableParams))
    },
    fetchItemFootprints (item) {
      this.api.getWorkerFootprints(item.id).then(res => {
    ...
    showResumeDialog (item) {
      this.$ext.mount(ResumeDialog, {onEl: this.$el, data: {owner: this, item}})
    },
    showCommentDialog (item) {
      this.$ext.mount(CommentDialog, {onEl: this.$el, data: {owner: this, item}})
    },
    destroy (row) {
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteWorker(row.id).then(res => {
