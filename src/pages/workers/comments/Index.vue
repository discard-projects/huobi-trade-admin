...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getWorkerComments(this.$root.item.id, this.mixTableParams))
    },
    fetchItemFootprints (item) {
      this.api.getWorkerCommentFootprints(this.$root.item.id, item.id).then(res => {
        this.footprints = res.data.items
      })
    },
    showNewDialog () {
      this.$ext.mount(New, {data: {owner: this, worker: this.$root.item}})
    },
    showEditDialog (item) {
      this.$ext.mount(Edit, {props: {item}, data: {owner: this, worker: this.$root.item}})
    },
    destroy (row) {
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteWorkerComment(this.$root.item.id, row.id).then(res => {
