  ...
  <div>
    <ex-table :tableData="mixTableData" @refetch="fetchData">
      ...
      <template slot="table-operations" slot-scope="{row, $index, intro}">
        ...
        <el-button size="mini" @click="showPermissionDialog(row)">管理权限</el-button>
...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getRoles(this.mixTableParams))
    },
    showNewDialog () {
      this.$ext.mount(New, {onEl: this.$el, data: {owner: this}})
    },
    showPermissionDialog (item) {
      this.$ext.mount(PermissionDialog, {onEl: this.$el, props: {item}, data: {owner: this}})
    },
    fetchItemFootprints (item) {
      this.api.getRoleFootprints(item.id).then(res => {
      ...
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteRole(row.id).then(res => {
