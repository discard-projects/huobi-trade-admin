...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getDicDiseases(this.mixTableParams))
    },
    showNewDialog () {
      ...
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteDicDisease(row.id).then(res => {
  ...
  created () {
    this.mixTableData = Object.assign(this.mixTableData, {
      ...
      dragable: (item, newPosition, oldPosition) => {
        this.api.updateDicDiseasePosition(item.id, {position: newPosition}).then(res => {
          this.fetchData()
        }).catch(err => {
          console.error(err)
          this.fetchData()
