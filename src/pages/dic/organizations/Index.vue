...
export default {
  ...
  methods: {
    fetchData () {
      this._fetchData(this.api.getDicOrganizations(this.mixTableParams))
    },
    showNewDialog () {
      ...
      this.$confirm('确定要删除该条信息吗？', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            ...
            this.api.deleteDicOrganization(row.id).then(res => {
  ...
  created () {
    this.mixTableData = Object.assign(this.mixTableData, {
      ...
      dragable: (item, newPosition, oldPosition) => {
        this.api.updateDicOrganizationPosition(item.id, {position: newPosition}).then(res => {
          this.fetchData()
        }).catch(err => {
          console.error(err)
          this.fetchData()
