...
export default {
    ...
    currentValue: {
      ...
      set (nv) {
        if (this.mutiple) {
          ...
          this.$emit('update:realUrl', nv['qr_real_link'])
  ...
  methods: {
    ...
    changeInput (ev) {
      if (ev.target.files.length) {
        ...
        this.api.uploadImage(fd).then(({data}) => {
          ...
          return this.decodeQrCode(data.link).then(decodeInfo => {
            data.qr_real_link = decodeInfo.result
    ...
    deleteImage () {
      this.currentValue = {link: '', signed_id: '', qr_real_link: ''}
    }
  },
  components: {
    [Upload.name]: Upload
